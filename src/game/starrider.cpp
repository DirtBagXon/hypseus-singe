...
// 0xD800-0xDFFF = RAM (6116 U7, no chip there, seems like a special case on
// schematics)
...
//		A0 : RS0 -> 0 = peripheral register or direction reg, 1 = control
//register
//		A1 : RS1 -> 0 = peripheral A (Video Graphics Generator), 1 = periperal B
//(PR-8210A)
...
starrider::starrider()
{
    ...
    // Since Star Rider doesn't work anyway, I got lazy and didn't add the ROM
    // loading info here
    // You can see the necessary info in the commented-out "load_roms" function
    // below ...
}
            ...
            for (int x = 0; x < 4; x++) {
                for (int y = 0; y < 8; y++) {
                    Uint8 left_pixel = static_cast<Uint8>(
                        (character[m_cpumem[chary * 64 + charx + 0x4000] * 32 + x + 4 * y] & 0xf0) >>
                        4);
                    Uint8 right_pixel = static_cast<Uint8>(
                        (character[m_cpumem[chary * 64 + charx + 0x4000] * 32 + x + 4 * y] & 0x0f));
                    *((Uint8 *)m_video_overlay[m_active_video_overlay]->pixels +
                      (((chary - 2) * 8 + y) * STARRIDER_OVERLAY_W) +
                      ((charx - 6) * 8 + x * 2)) = left_pixel;
                    *((Uint8 *)m_video_overlay[m_active_video_overlay]->pixels +
                      (((chary - 2) * 8 + y) * STARRIDER_OVERLAY_W) +
                      ((charx - 6) * 8 + x * 2 + 1)) = right_pixel;
                }
            }
        }
    }
