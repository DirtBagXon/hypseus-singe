...
void lgp::do_nmi() { Z80_ASSERT_NMI; }
...
void lgp::draw_8x8(int character_number, int xcoord, int ycoord)
{
    ...
    for (int y = 0; y < 8; y++) {
        if ((y + ycoord) < LGP_OVERLAY_H) {
            ...
            pixel[0] =
                static_cast<Uint8>(((byte4 & 0x80) >> 4) | ((byte3 & 0x80) >> 5) |
                                   ((byte2 & 0x80) >> 6) | ((byte1 & 0x80) >> 7));
            pixel[1] =
                static_cast<Uint8>(((byte4 & 0x40) >> 3) | ((byte3 & 0x40) >> 4) |
                                   ((byte2 & 0x40) >> 5) | ((byte1 & 0x40) >> 6));
            pixel[2] =
                static_cast<Uint8>(((byte4 & 0x20) >> 2) | ((byte3 & 0x20) >> 3) |
                                   ((byte2 & 0x20) >> 4) | ((byte1 & 0x20) >> 5));
            pixel[3] =
                static_cast<Uint8>(((byte4 & 0x10) >> 1) | ((byte3 & 0x10) >> 2) |
                                   ((byte2 & 0x10) >> 3) | ((byte1 & 0x10) >> 4));
            pixel[4] =
                static_cast<Uint8>(((byte4 & 0x08) >> 0) | ((byte3 & 0x08) >> 1) |
                                   ((byte2 & 0x08) >> 2) | ((byte1 & 0x08) >> 3));
            pixel[5] =
                static_cast<Uint8>(((byte4 & 0x04) << 1) | ((byte3 & 0x04) << 0) |
                                   ((byte2 & 0x04) >> 1) | ((byte1 & 0x04) >> 2));
            pixel[6] =
                static_cast<Uint8>(((byte4 & 0x02) << 2) | ((byte3 & 0x02) << 1) |
                                   ((byte2 & 0x02) << 0) | ((byte1 & 0x02) >> 1));
            pixel[7] =
                static_cast<Uint8>(((byte4 & 0x01) << 3) | ((byte3 & 0x01) << 2) |
                                   ((byte2 & 0x01) << 1) | ((byte1 & 0x01) << 0));

            for (int x = 0; x < 8; x++) {
                if ((pixel[x]) && ((x + xcoord) < LGP_OVERLAY_W)) {
                    *((Uint8 *)m_video_overlay[m_active_video_overlay]->pixels +
                      ((ycoord + y) * LGP_OVERLAY_W) + (xcoord + x)) =
                        ((Uint8)pixel[x]);
                }
            }
        }
