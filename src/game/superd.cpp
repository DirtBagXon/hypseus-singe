...
// Upon receiving LD-V1000 status strobe, the LD-V1000 status is immediately
// stored into
//  a latch which can then be read at any time by reading z80 port 4.
// Upon writing to z80 port 0, a byte is put into a latch which is always sent
// to
...
struct addr_name superd_addr_names[] = {{"Start", 0x0000},
                                        {"SdqStart", 0x0210},
                                        {"MainGameLoop", 0x02A0},
                                        ...
                                        {"FrameCalcPtr", 0x40E2},
                                        {"Joystick0",
                                         0x40F0}, // here is where info from
                                                  // input port 0 (joystick) get
                                                  // stored
                                        {"Joystick1", 0x40F1},
                                        {"JoyChanged?1", 0x40F2},
                                        {"JoyChanged?2", 0x40F3},
...
superd::superd()
{
    ...
    static struct rom_def roms[] = {// main z80 program
...
bool superd::init()
{
    ...
    // video overlay
    // hardware (apparently) but at this time, I can't find any other way to
    // blank the
    // screen during skips.
    // NOTE : some users may not like the blanking so we might want to allow
    // them to turn
...
// Called whenever the emulator wants to output to a port
{
    ...
    case 0x04:
        if (!m_prefer_samples) {
            ...
            // The sound chip sends a bunch of data to port 4 to control the
            // sound chip.
            // All we do here is look for unique data to figure out which sample
            // we should play.
            // It turns out that only 8's are sent for the coin sound, only 11's
            // are sent for the
            // successful move sound, and only 0xC1 is send for the bad move
            // sound.
            // Also, about eight 0x11's are sent for the successful move, so we
            // make sure we only
            // play a sound for 1 out of every eight 0x11's we get.  Same type
            // of deal for the coin
            // sound.
            if (Value == 0x08) {
                if (snd_coin_count == 0) {
            ...
            // okay)
            // you get more points and a higher-pitched sound if your reaction
            // is quick
            if (Value == 0x12) {
                if (snd_succeed_count == 0) {
            ...
            for (int x = 0; x < 4; x++) {
                for (int y = 0; y < 8; y++) {
                    Uint8 left_pixel = static_cast<Uint8>(
                        (character[m_cpumem[chary * 32 + charx + 0x5c00] * 32 + x + 4 * y] & 0xf0) >>
                        4);
                    Uint8 right_pixel = static_cast<Uint8>(
                        (character[m_cpumem[chary * 32 + charx + 0x5c00] * 32 + x + 4 * y] & 0x0f));
                    *((Uint8 *)m_video_overlay[m_active_video_overlay]->pixels +
                      ((chary * 8 + y) * SUPERD_OVERLAY_W) + (charx * 8 + x * 2)) = left_pixel;
                    *((Uint8 *)m_video_overlay[m_active_video_overlay]->pixels +
                      ((chary * 8 + y) * SUPERD_OVERLAY_W) +
