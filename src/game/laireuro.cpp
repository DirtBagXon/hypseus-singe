...
Uint8 ctc_read(Uint8 channel) { return g_ctc.channels[channel].time_const; }
...
void ctc_update_period(Uint8 channel)
{
    ...
    if (g_ctc.channels[channel].mode == TIMER) {
        new_period = g_ctc.channels[channel].time_const *
                     g_ctc.channels[channel].prescaler * g_ctc.clock;
    } else {
