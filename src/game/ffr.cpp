...
// just a little exercise to try to see if I can get a clip from the Freedom
// Fighter laserdisc
...
struct clip_boundary ffr_clips[] = {
    ...
    {3557, 3568}, {3589, 3598}, {3617, 3626}, {3665, 3670}, {3707, 3712},
    {3749, 3756}, {3797, 3802}, {3841, 3846}, {3883, 3890}, {3929, 3934},
    {3975, 3980}, {4017, 4024}, {4063, 4068}, {4109, 4114}, {4151, 4158},
    {4197, 4202}, {4243, 4248}, {4285, 4292}, {4331, 4336}, {4377, 4382},
    {4419, 4426}, {4465, 4470}, {4511, 4516}, {4553, 4560}, {4599, 4604},
    {4647, 4650}, {0, 0}};

// super don test clip
struct clip_boundary sdq_clips[] = {{5900, 5984},
                                    {6065, 6300},
                                    ...
                                    {11148, 11160}, // rom goes to 11147 which
                                                    // is a bad frame
                                    {16100, 16162},
                                    ...
                                    {17058, 17100}, // rom 17057 (1 frame too
                                                    // early)
                                    {0, 0}};

struct clip_boundary sdqshort_clips[] = {{5961, 5984}, {6065, 6075}, {0, 0}};
...
void ffr::start()
{
    ...
    // new behavior: loop until user quits so we can test this thing
    // exhaustively
    while (!get_quitflag()) {
        finished = false;
        index    = 0;
        ...
        while (!finished && !get_quitflag()) {
            ...
            if (m_pClips[index + 1].start) {
                g_ldp->pre_skip_forward(
                    (Uint16)(m_pClips[index + 1].start - m_pClips[index].end));
            } else {
                finished = true;
            }
...
void ffr::think()
{
    static const unsigned int uTICK_MS = 1; // how many ms equals 1 virtual
                                            // millisecond (this can be used to
                                            // slow things down)
    ...
    if (uDiff > uTICK_MS) {
        ...
        // if we're really lagged, it probably means we got interrupted, so just
        // set the timer equal ...
        if (uDiff > 60) {
            m_uTimer = uTicks;
        }
