...
// BANK 1
// Enabling both of these gives you 3 extra lives to what you normally get (3).
// Hence 6 lives.
...
class cliff : public game
{
  ...
  protected:
    char m_frame_str[FRAME_ARRAY_SIZE]; // current frame of laserdisc in string
                                        // form
    Uint16 m_frame_val; // current frame of the laserdisc in numerical form (0
                        // means busy/stopped)
  ...
  private:
    ...
    // used to make sure that a sound that is already playing doesn't get played
    // again
