...
#ifdef SAVE_RCSID
static char rcsid =
 "@(#) $Id: SDL_dx5yuv.c 1815 2005-09-24 21:07:13Z matt $";
#endif
...
int DX5_DisplayYUVOverlay(_THIS, SDL_Overlay *overlay, SDL_Rect *dstrect)
{
	...
	if ((dstrect->w != previous.w) || (dstrect->h != previous.h) ||
		(dstrect->x != previous.x) || (dstrect->y != previous.y))
	{
		previous.x = dstrect->x;
		previous.y = dstrect->y;
		previous.w = dstrect->w;
		previous.h = dstrect->h;
	...
	if (g_overlay_needs_updating)
	{
		...
		result = IDirectDrawSurface3_UpdateOverlay(surface, &src,
				SDL_primary, &dst, overlay->hwdata->overlay_flags,
