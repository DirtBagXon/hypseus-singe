...
char pr7820_frame[FRAME_ARRAY_SIZE] = {0}; // holds the digits sent to the
                                           // PR-7820
...
bool read_pr7820_ready()
{
    ...
    if (g_bPR7820SearchPending) {
        ...
        // search failed for whatever reason ...
            ...
            g_pr7820_ready = 1; // leave it as 1 (not ready) to indicate a
                                // problem
            g_bPR7820SearchPending = false;
        }
    ...
    else if (g_pr7820_autostop_frame != 0) //  none the games actually need
                                           //  autostop anyway...
    {
        //			char f[81];
...
void pr7820_clear(void) { pr7820_frame_index = 0; }
...
void reset_pr7820()
{
    ...
    g_pr7820_ready = 1; // PR-7820 is PARK'd and NOT READY (when stopped it's
                        // not ready)
}
