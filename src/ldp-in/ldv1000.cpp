...
char ldv1000_frame[FRAME_ARRAY_SIZE] = {0}; // holds the digits sent to the
                                            // LD-V1000

unsigned char g_ldv1000_output = 0xFC; // LD-V1000 is PARK'd and READY

bool g_ldv1000_search_pending = false; // whether the LD-V1000 is currently in
                                       // the middle of a search operation or
                                       // not (NOT USED WITH BLOCKING SEEKING)
Uint64 g_ldv1000_search_begin_cycles =
    0; // the current cycle count when LD-V1000 began searching (used to force
       // it to be busy for a certain period of time)
Uint32 g_ldv1000_cycles_per_search = 0;    // how many cycles a read operation
                                           // should last (some games require a
                                           // simulated delay)
double g_ldv1000_seconds_per_search = 0.5; // minimum # of seconds that a search
    ...
    if (g_ldv1000_output_stack_pointer < 1) {
        ...
        if (g_ldv1000_search_pending) {
            ...
            Uint32 elapsed_cycles =
                (Uint32)(get_total_cycles_executed(0) - g_ldv1000_search_begin_cycles);
...
void write_ldv1000(unsigned char value)
{
    ...
    if (g_ldv1000_output & 0x80) {
        ...
        case 0xBF: // clear
            clear();
            g_ldv1000_output &= 0x7F; // Super Don expects highbit to get
        ...
        case 0xA3: // play at 1X
            ...
            if (g_ldp->get_status() != LDP_PLAYING) {
                ...
                g_ldv1000_output = 0x2e; // not ready and in FORWARD (variable
                                         // speed) mode
            }
        ...
        case 0xF3: // Auto Stop (used by Esh's)
            ...
            {
                LOGD << fmt("Auto-Stop requested at frame %u", g_ldv1000_autostop_frame);
            }
            break;
        case 0xFD: // Play
            g_ldp->pre_play();
        ...
        case 0xF7: // Search
            ...
            // else if we are to use the more stable blocking seeking method ...
                ...
                if (!g_instant_seeking) {
                    ldv1000_stack_push(0x50); // 0x50 means we are busy
                                              // searching
...
void clear(void) { memset(ldv1000_frame, 0, FRAME_SIZE); }
...
// To get accurate strobe values, simply call ldv1000_report_vsync on every
// vsync pulse
...
Uint32 g_ldv1000_until_status_start = 0; // # of cpu cycles until status strobe
                                         // begins
Uint32 g_ldv1000_until_status_end = 0;   // # of cpu cycles u ntil status strobe
                                         // ends
Uint32 g_ldv1000_until_command_start = 0; // # of cpu cycles until command
                                          // strobe begins
Uint32 g_ldv1000_until_command_end = 0; // # of cpu cycles until command strobe
                                        // ends
...
{
    g_ldv1000_last_event = (uint64_t)eventType; // changed to long for x64
...
#endif
        cpu_set_event(0, g_ldv1000_until_status_start, ldv1000_event_callback,
...
#endif
        cpu_set_event(0, g_ldv1000_until_status_end, ldv1000_event_callback,
...
#endif
        cpu_set_event(0, g_ldv1000_until_command_start, ldv1000_event_callback,
...
#endif
        cpu_set_event(0, g_ldv1000_until_command_end, ldv1000_event_callback,
    ...
    case LDV1000_EVENT_COMMAND_END:
        ...
        break;
    default:
        LOGW << "unhandled ldv1000 event, fix this!";
        set_quitflag();
        break;
    }
}
...
#endif
    cpu_set_event(0, g_ldv1000_until_vsync_end, ldv1000_event_callback,
...
{
    LOGD << fmt("The last LD-V1000 event was %d", g_ldv1000_last_event);
}
