...
bool v6000::nonblocking_search(char *frame)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        if (index <= 9) {
            ...
            LOGW << "Bug in v6000_search function";
        }
    } // end for loop
    serial_tx('F');
    serial_tx('7'); // search command
...
bool v6000::skip_forward(Uint16 frames_to_skip, Uint16 target_frame)
{
    ...
    if (frames_to_skip < 100) {
        ...
        LOGW << "Cannot skip more than 100 frames!";
    }
...
unsigned int v6000::play()
{
    ...
    LOGD << "Sending play command to the LD-V6000";
    while (!success && attempts < max_attempts) {
        serial_tx('F');
        serial_tx('D');
...
void v6000::stop()
{
    ...
    // We have to check to see whether the disc is playing before we send a
    // reject
    // command because if the disc is not playing, the player will eject the
    // disc.
    serial_rxflush();
    serial_tx('D');
    serial_tx('4');
    ...
    if (strcmp(status, "64") == 0) {
        ...
        LOGD << fmt("Ignoring stop command because disc is not playing -> %s", status);
    }
}
...
// checks LD-V6000 and waits until it is finished either searching or spinning
// up to play
...
// or a 0 if there is an error
{
    ...
    // keep querying the LD-V6000 for its status until it is finished searching
    ...
    while ((!finished) && (finish_timer < time_to_finish)) {
        ...
        if (gotstatus) {
            ...
            // if the disc is spinning up
                ...
                if (!spun_up_before) {
                    spun_up_before = true;
                    LOGD << "Waiting for disc to spin up...";
                }
                finish_timer = 0;
            }
            ...
            // if the LDP is spinning down
                ...
                LOGW << fmt("Unknown result code: %s", s);
            ...
            LOGD << fmt("Got garbage result from player: %s", s);
        }
...
// watchquit is whether to abort if the quitflag is set (set it to true if you
// aren't sure)
...
bool v6000::getstring(char *s, Uint32 timeout, bool watchquit)
{
    ...
    while ((index < 4) && !(watchquit && get_quitflag())) {
        ...
        if (elapsed_ms_time(cur_time) >= timeout) {
            LOGW << "timed out waiting for a response";
            break;
        }
        SDL_check_input();
    } // end while
