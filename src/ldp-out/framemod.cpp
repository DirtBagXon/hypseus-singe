...
// contains functions to convert the regular DL/SA NTSC frames to other formats
// such as PAL
...
unsigned long get_frame_conversion_fpks()
{
    ...
    default:
        printline(
            "Error in get_frame_conversion_fpks, no frame modifier is enabled");
        break;
    };
...
Uint32 do_frame_conversion(long source_frame)
{
    ...
    // if we are using space ace 91 we need to convert from 24 FPS to 30 FPS and
    // add a time constant
    if (get_frame_modifier() == MOD_SA91) {
        ...
        // if frame to be modified is greater than 145
            ...
            result_frame += 7.80807717679; // add time difference between SA '83
                                           // and SA '91
            result_frame *= 29.97;         // convert seconds to SA91 frames
            result_frame += 0.5; // add  0.5 so when truncating, the average
                                 // becomes the resulting frame
        }
    } // end if Space Ace 91
    ...
    // regular Dragon's Lair PAL conversion
        ...
        if (result_frame < 1) {
            result_frame = 1;
            printline(
                "NOTE: NTSC frame requested is not available on PAL DL disc");
        }
    }

    // Space Ace PAL conversion
    else if (get_frame_modifier() == MOD_PAL_SA) {
        result_frame = result_frame * (25.0 / 23.976); // convert NTSC frames to
                                                       // PAL frames
        // NTSC and PAL discs have a 1:1 time correspondance
        result_frame += 0.5; // add 0.5 so when truncating, the average becomes
                             // the resulting frame
    }
    ...
    // Dragon's Lair PAL for the Software Corner conversion
        ...
        if (result_frame < 1) {
            result_frame = 1;
            printline("NOTE: NTSC frame requested is not available on DL "
