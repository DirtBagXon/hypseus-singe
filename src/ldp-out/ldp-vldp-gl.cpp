...
// for speed
extern bool g_take_screenshot; // if true, a screenshot will be taken at the
...
// last frame index to be requested to be displayed, and last frame index that
// was displayed
// (index corresponds to g_uFramePrepReq)
unsigned int g_uFrameDispReq = 0;
unsigned int g_uFrameDispAck = 0;

// blank requested means the next frame we display should be blank, with overlay
// drawn on top
...
// Macros to lock and unlock the mutex to make sure two threads aren't accessing
// shared vars at the same time
...
const GLchar *g_FProgram = "uniform sampler2D Ytex;\n"
...
// 8-bit texture fragment program (by Matt Ownby, my first fragment shader!
// woohoo!)
const GLchar *g_F8Bit =
    ...
    "void main(void) {\n"
    "  float idx;\n"
    // idx gets a value between 0.0 and 1.0 which corresponds to 0-255 on our
    // color palette
    "  idx=texture2D(smpPixels, gl_TexCoord[0].st).r;\n"
    // 255/256 (0.99609375) converts 0.0-1.0 to the left border of the color
    // that we want in the palette texture
    // Adding 1/512 (0.001953125) centers us inside the color that we want so
    // that there can be no ambiguity
    "  gl_FragColor=texture1D(smpColorPalette, (idx * 0.99609375) + "
    "0.001953125);\n"
    "}\n";
...
void ldp_vldp_gl_blend_y(Uint8 *g_ptrY)
{
    ...
    // I just coded it up quickly to make sure it is supported to save myself
    // any questions
...
void draw_prepared_frame(SDL_Surface *gamevid)
{
    ...
    if (gamevid) {
        ...
        // The palette texture has already been copied over in
        // on_palette_change_gl()

        // update 8-bit pixels to texture
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, g_textureIDs[TEX_VID_OVERLAY]);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, gamevid->w, gamevid->h, 0,
                     GL_LUMINANCE, GL_UNSIGNED_BYTE, gamevid->pixels);
        ...
        glBegin(GL_QUADS);
        glTexCoord2f(0, 1);
        glVertex3i(-w_half, iY, 1); // bottom left
        glTexCoord2f(1, 1);
        glVertex3i(w_half, iY, 1); // bottom right

        iY += g_uOverlayHeight;

        glTexCoord2f(1, 0);
        glVertex3i(w_half, iY, 1); // top right
        glTexCoord2f(0, 0);
        glVertex3i(-w_half, iY, 1); // top left
        glEnd();
        glPopMatrix();
    ...
    if (g_filter_type & FILTER_SCANLINES) {
        ...
        glBegin(GL_QUADS);
        glTexCoord2i(0, g_uVidHeight >> 1);
        glVertex3i(-w_half, -h_half, 2);
        glTexCoord2i(1, g_uVidHeight >> 1);
        glVertex3i(w_half, -h_half, 2);
        glTexCoord2i(1, 0);
        glVertex3i(w_half, h_half, 2);
        glTexCoord2i(0, 0);
...
void ldp_vldp_gl_think(unsigned int uVblankCount)
{
    ...
    if (g_uFrameDispReq > g_uFrameDispAck) {
        ...
        // Use the other texture array so that we can buffer our next YUV image
        // and
        //  still allow access to the current one in case the video overlay
        //  changes before
        ...
        // this allows us to display frames that are slightly behind but
        // buffered
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, g_uTexWidth >> 1, g_uTexHeight >> 1,
                     0, GL_LUMINANCE, GL_UNSIGNED_BYTE, g_ptr[uIdx][U_IDX]);

        glActiveTexture(GL_TEXTURE2);
        glBindTexture(GL_TEXTURE_2D, g_textureYUVIDs[uNextTextureArray][TEX_V]);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, g_uTexWidth >> 1, g_uTexHeight >> 1,
                     0, GL_LUMINANCE, GL_UNSIGNED_BYTE, g_ptr[uIdx][V_IDX]);

        // if blending is requested, then blend the Y plane now before sending
        // it to the texture
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, g_uTexWidth, g_uTexHeight,
                     0, GL_LUMINANCE, GL_UNSIGNED_BYTE, g_ptr[uIdx][Y_IDX]);

        g_uCurTextureArray ^= 1; // flip texture array so we're using our new
                                 // VLDP frame when we draw

        bOkToDraw = true; // it's ok to draw the frame now ...
    }

    // if our ROM-generated video overlay has changed
    // (THIS SHOULD NOT BE AN 'ELSE IF' BECAUSE WE WANT TO UPDATE BOTH AT THE
    // SAME FRAME IF NEEDED DUE TO VSYNC DELAY)
    ...
    // (We don't want to draw every time bOkToDraw is true because if the
    // laserdisc video
    //  changes at 30 FPS and the overlay changes at 60 FPS, we will have
    //  updates at 90 FPS
    //  which is too fast for most monitor refresh rates, although it does work
    //  fine with
    //  vsync disabled)
    if ((uVblankCount != g_uVblankCountOld) && (bOkToDraw)) {
        // if we're not too far behind, then display the frame
        // I chose 33 because it is approximately 2 frames on a 60 Hz monitor,
        // and 1 NTSC frame
        if (g_uCPUMsBehind < 33) {
            // draw it! (this should go really fast since the textures are
            // already set up)
        ...
        // else we're too far behind and SDL_GL_SwapBuffers can take a long time
        // if vsync is enabled,
        //  so drop this frame to try to catch up.

        // TODO : maybe we should show every 4th frame even if we are dropping
        // just to avoid
        //  having 'dead' air
...
bool init_vldp_opengl()
{
    ...
    if (!g_bOpenGLInitialized) {
        ...
        for (i = 0; i < NUM_TEXTURES; i++) {
            ...
            GLuint uParam = GL_CLAMP_TO_EDGE; // no reason to ever use GL_CLAMP
                                              // based on what I've now learned
                                              // :)
            GLenum uTarget = GL_TEXTURE_2D;

            switch (i) {
            // the color palette texture is 1-Dimensional
            // We _must_ use GL_NEAREST for TEX_VID_PALETTE because it is a
            // lookup table.
            ...
            case TEX_SCANLINES:
                uParam = GL_REPEAT;
                break;
            // IMPORTANT: For TEX_VID_OVERLAY, we _must_ use GL_NEAREST for our
        ...
        for (i = 0; i < NUM_YUV_TEXTURES; i++) {
            GLenum uTarget = GL_TEXTURE_2D;
            GLuint uParam  = GL_CLAMP_TO_EDGE; // this solves the 'green border'
                                               // problem (GL_CLAMP was causing
                                               // it)
            GLuint uType = GL_LINEAR;
        ...
        // NOW CREATE SCANLINES TEXTURE (it is so small it's ok to create it
        ...
        if (init_shader()) {
            ...
            if (g_vldp_gl_mutex) {
                // IMPORTANT: this must be set to true before
                // on_palette_change_gl is called!!!
                g_bOpenGLInitialized = true;

                // This is important for us to do here, because the palette may
                // have been finalized
                //  before we initialized VLDP GL, and it may never change
                //  hereafter.  Therefore,
...
{
    // this function may get called before we've initialized our GL stuff, in
    // which
    //  case, we will have to ignore it.
    if (g_bOpenGLInitialized) {
        // These shouldn't be necessary to do here, but I've left them here,
        // commented out,
        //  in case problems are discovered on other hardware and they are
        //  needed.
        //		glActiveTexture(GL_TEXTURE1);
        //		glBindTexture(GL_TEXTURE_1D, g_textureIDs[TEX_VID_PALETTE]);
        ...
        glTexImage1D(GL_TEXTURE_1D, 0, GL_RGBA, 256, 0, GL_RGBA,
                     GL_UNSIGNED_BYTE, g_uRGBAPalette);
    }
...
int prepare_frame_GL_callback(struct yuv_buf *src)
{
    ...
    /*
    // used to test super don overflow problem, can probably be removed once
    we're
    ...
    */
    ...
    // NOTE : we have multiple YUV buffers because, due to threading, it's
    // possible
    //  for these callbacks to get called many times before the gl_think
    //  function
    //  is called.
    // One scenario: prepare callback, display callback, followed by another
    // prepare
    //  callback.  Now we have 2 frames prepared, 1 frame requested to be
    //  displayed,
    //  and our gl_think hasn't even run yet!  We need a way to keep track of
    //  all
    ...
    // If a prepared frame hasn't been displayed, then we should overwrite it
...
void render_blank_frame_GL_callback()
{
    ...
    // (we don't need to populate the YUV buffer because the frame will be
    // blank)
    ++g_uFramePrepReq;
    ++g_uFrameDispReq;
