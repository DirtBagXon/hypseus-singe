...
bool pioneer::init_player()
{
    ...
    if (success) {
        ...
        printline("Pioneer error, CL command not support (you probably aren't "
                  "communicating with your player)");
    }
...
int pioneer::get_search_result()
{
    ...
    // we don't want to spend any time waiting for a result unless we have one
    // to wait for
    if (serial_rx_char_waiting()) {
        // the result isn't complete until it gets a CR/LF at the end.
        // We therefore want to wait until we get the CR/LF, which we assume
        // will be instant,
        // but it is safe to put a long timeout anyway even though we never
        // expect to wait
        // very long.
        ...
        serial_rxflush(); // clear out other characters from receive buffer
                          // (such as LF or CR)
    }
    return result;
}
...
// Tests if player can skip_forward and if can't uses the generic search
// function
bool pioneer::skip_forward(Uint16 frames_to_skip, Uint16 target_frame)
{
    ...
    if (skipping_supported) {
        ...
        sprintf(frame, "%d", frames_to_skip); // convert number of frames to
                                              // string
...
void pioneer::printmodel()
{
    ...
    if (gotid) {
        if (strcasecmp(model, "P1527") == 0) {
            ...
            skipping_supported = true; // Only the 4400 and 8000 support the
                                       // skip command
