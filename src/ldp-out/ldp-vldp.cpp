                    ...
                    if (g_vldp_info != NULL) {
                        ...
                        if (g_vldp_info->uApiVersion == API_VERSION) {
                            ...
                            if (m_testing) {
                                ...
                                printline(
                                    "Run releasetest to see printed results!");
                                set_quitflag();
                            }
                            ...
                            if (bPreCacheOK) {
                                ...
                                if (open_and_block(m_mpeginfo[0].name)) {
                                    ...
                                    printline("LDP-VLDP ERROR : first video "
                        ...
                        printline("LDP-VLDP ERROR : vldp_init returned NULL "
...
void ldp_vldp::shutdown_player()
{
    ...
    if (is_sound_enabled()) {
        if (!delete_soundchip(m_uSoundChipID)) {
            printline("ldp_vldp::shutdown_player WARNING : sound chip could "
                      "not be deleted");
        }
    }
    free_vldp_lib();
...
bool ldp_vldp::nonblocking_search(char *frame)
{
        ...
        if (result) {
            ...
            if (!need_frame_conversion()) {
                ...
                if (g_game->get_disc_fpks() != uFPKS) {
                    string s = "NOTE: converting FPKS from " +
                               numstr::ToStr(g_game->get_disc_fpks()) + " to " +
                               numstr::ToStr(uFPKS) +
    ...
    // else mpeg_info() wasn't able to provide a filename ...
        ...
        printline("LDP-VLDP.CPP ERROR: frame could not be converted to file, "
...
unsigned int ldp_vldp::play()
{
    ...
    if (m_cur_mpeg_filename == "") {
        ...
        if (bOK) {
            ...
            outstr(
                "LDP-VLDP.CPP : in play() function, could not open mpeg file ");
...
bool ldp_vldp::skip_forward(Uint16 frames_to_skip, Uint16 target_frame)
{
    ...
    if (uDiscFPKS == uFPKS) {
        ...
        if (!g_vldp_info->uses_fields) {
            ...
            printline("LDP-VLDP ERROR : Skipping not supported with mpegs that "
                      "use fields (such as this one)");
        }
    } else {
        string s = "LDP-VLDP ERROR : Skipping not supported when the mpeg's "
                   "framerate differs from the disc's (" +
                   numstr::ToStr(uFPKS / 1000.0) + " vs " +
                   numstr::ToStr(uDiscFPKS / 1000.0) + ")";
        printline(s.c_str());
    }
...
{
#ifdef DEBUG
    string s = "ldp_vldp::pause() : g_vldp_info's current frame is " +
...
bool ldp_vldp::change_speed(unsigned int uNumerator, unsigned int uDenominator)
{
    ...
    // else the audio should be played at the correct location
        ...
        if (seek_audio(u64AudioTargetPos)) {
            ...
            printline(
                "WARNING : trying to seek audio after playing at 1X failed");
        }
    }
...
unsigned int ldp_vldp::get_current_frame()
{
    ...
    if ((unsigned int)result != m_uCurrentFrame) {
        ...
        if ((unsigned int)result > m_uCurrentFrame) {
            string s = "ldp-vldp::get_current_frame() [vldp ahead]: internal "
                       "frame is " +
                       numstr::ToStr(m_uCurrentFrame) +
                       ", vldp's current frame is " + numstr::ToStr(result);
            printline(s.c_str());

            s = "g_local_info.uMsTimer is " + numstr::ToStr(g_local_info.uMsTimer) +
                ", which means frame offset " +
                numstr::ToStr((g_local_info.uMsTimer * uFPKS) / 1000000);
            s += " (" + numstr::ToStr(g_local_info.uMsTimer * uFPKS * 0.000001) + ") ";
            printline(s.c_str());
            s = "m_uCurrentOffsetFrame is " + numstr::ToStr(m_uCurrentOffsetFrame) +
                ", m_last_seeked frame is " + numstr::ToStr(m_last_seeked_frame);
            printline(s.c_str());
            ...
            if (mpo_read(ff_buf, (unsigned int)p_ioFileConvert->size,
                ...
                if (bytes_read == p_ioFileConvert->size) {
                    ...
                    if (parse_framefile((const char *)ff_buf, framefile_path.c_str(),
                                        ...
                                        err_msg)) {
                        outstr("Framefile parse succeeded. Video/Audio "
                               "directory is: ");
                    ...
                    printerror("ldp-vldp.cpp : framefile read error");
            } else
                printerror("ldp-vldp.cpp : framefile read error");
        } else
            printerror("ldp-vldp.cpp : mem alloc error");
...
bool ldp_vldp::precache_all_video()
{
    ...
    if (bResult) {
        ...
        if ((uReqMegs < uMegs) || (m_bPreCacheForce)) {
            ...
            printline(((string) "Not enough memory to precache video stream.  "
                                "You have about " +
                       numstr::ToStr(uMegs) + " but need " + numstr::ToStr(uReqMegs))
                          .c_str());
            bResult = false;
        }
    }
...
int prepare_frame_callback_with_overlay(struct yuv_buf *src)
{
    ...
    // However, it is also fast and therefore we just need to be conscious
    coders and not let endless loops happen
    ...
    */
    ...
    if (SDL_LockYUVOverlay(g_hw_overlay) == 0) {
        ...
        if ((gamevid->w << 1) == g_hw_overlay->w) {
            ...
            unsigned int row = 0;
            unsigned int col = 0;
            Uint32 w_double  = g_hw_overlay->w << 1; // twice the overlay width,
                                                    // to avoid calculating this
                                                    // more than once
            Uint32 h_half = g_hw_overlay->h >> 1; // half of the overlay height,
                                                  // to avoid calculating this
                                                  // more than once
            Uint8 *dst_ptr;
            ...
            unsigned int channel0_pitch =
                g_hw_overlay->pitches[0]; // this val gets used a lot so we put
                                          // it into a var
            ...
            for (row = 0; row < h_half; row++) {
                ...
                // if we're doing scanlines
                    ...
                    if (g_filter_type & FILTER_BLEND) {
                        ...
                        memcpy(dst_ptr + channel0_pitch, g_line_buf,
                               (g_hw_overlay->w << 1)); // this could be
                                                        // g_line_buf2 also
                    }
                }
                ...
                dst_ptr += (channel0_pitch << 1); // we've done 2 rows, so skip
                                                  // a row
                Y += g_hw_overlay->w; // we've done 2 vertical Y pixels, so skip
                                      // a row
                Y2 += g_hw_overlay->w;
            }
            ...
            if (!warned) {
                ...
                if (!g_game->is_overlay_size_dynamic()) {
                    ...
                    sprintf(s, "Your MPEG's size is %d x %d, and needs to be "
                               "%d x %d",
                            g_hw_overlay->w, g_hw_overlay->h, (gamevid->w << 1),
                            (gamevid->h << 1));
                    printline(s);
                }
...
// (which Xbox-Daphne may be using)
// copies the contents of src into dst
// assumes destination overlay is locked and *IMPORTANT* assumes src and dst are
// the same resolution
    ...
    for (row = 0; row < (dst->h >> 1); row++) {
        ...
        if (!(g_filter_type & FILTER_SCANLINES)) {
            ...
            if (!(g_filter_type & FILTER_BLEND)) {
                ...
                g_blend_func(); // blend the two lines into g_line_buf3
                // this won't affect video overlay because it is already doubled
                // in size anyway
        ...
        // if we're doing scanlines
            ...
            // (on nvidia it makes the top line too bright if the black line
            // doesn't come first!)
            for (int i = 0; i < (dst->w << 1); i += 4) {
                *((Uint32 *)(dst_ptr + i)) = YUY2_BLACK; // this value is black
                                                         // in YUY2 mode
            }
...
void update_parse_meter()
{
    ...
    if (g_dPercentComplete01 >= 0) {
        ...
        if (remaining_s > 0) {
            ...
            SDLDrawText(s, screen, FONT_SMALL,
                        (half_w - ((strlen(s) / 2) * FONT_SMALL_W)), half_h - FONT_SMALL_H);
...
void report_mpeg_dimensions_callback(int width, int height)
{
    ...
    if (!g_hw_overlay) {
        ...
        g_hw_overlay = SDL_CreateYUVOverlay(width, height - (g_vertical_stretch * 4),
                                            SDL_YUY2_OVERLAY, get_screen());
        ...
        // we don't need to check whether these buffers have been allocated or
        // not because this is checked for earlier
        ...
        memset(g_blank_yuv_buf.U, 127, g_blank_yuv_buf.UV_size); // blank U
                                                                 // color
        g_blank_yuv_buf.V = MPO_MALLOC(g_blank_yuv_buf.UV_size);
        memset(g_blank_yuv_buf.V, 127, g_blank_yuv_buf.UV_size); // blank V
                                                                 // color
    ...
    // This is some one-time setup stuff that will be used by the blending
    // functions
    // It will be used by both the prepare w/ overlay and prepare w/o overlay
    // functions
    if (g_filter_type & FILTER_BLEND) {
