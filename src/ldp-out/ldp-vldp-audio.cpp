...
{
#ifdef DEBUG
    assert(bytes_to_copy % 4 == 0); // stereo 16-bit audio should always be
                                    // divisible by 4
#endif
    Uint32 *dst32   = (Uint32 *)dest;
    Uint16 *src16_L = (Uint16 *)src; // point to first bit of left-channel data

    bytes_to_copy >>= 2; // divide by 4 because we will be advancing 4 bytes per
                         // iteration
...
void *audiocopy_right_only(void *dest, const void *src, size_t bytes_to_copy)
{
    ...
    Uint32 *dst32   = (Uint32 *)dest;
    Uint16 *src16_R = (Uint16 *)src + 1; // point to the first bit of data that
                                         // occurs on the right channel

    bytes_to_copy >>= 2; // divide by 4 because we will be advancing 4 bytes per
                         // iteration
    for (unsigned int index = 0; index < bytes_to_copy; index++) {
        // fastest method (third attempt)
        *dst32 = (*src16_R << 16) | *src16_R; // we only care about the
                                              // right-channel
        dst32++;
...
// sets the audio copy callbackto be used, based on which audio channels are
// muted
...
{
    bool result              = false;
    ov_callbacks mycallbacks = {mmread, mmseek, mmclose, mmtell};
