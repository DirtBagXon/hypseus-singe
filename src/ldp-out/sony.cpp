...
// we assume that this function will only get called after a search has been
// issued
int sony::get_search_result()
{
    ...
    if (serial_rx_char_waiting()) {
        ...
        // if we get any other character, then we can assume that the search
        // failed
        else {
...
// (if you get errors querying for frame number, use the generic function
instead)
