...
{
	unsigned int tmp = CompressFlags();
	ICOUNT -= cycles.pushf;
#ifdef I286
    PUSH( tmp | 0xc000 );
#elif defined V20
...
static void PREFIX86(_popf)(void)    /* Opcode 0x9d */
{
	...
	if (I.IF && I.irq_state)
#ifdef V20
		PREFIX(_interrupt)((unsigned int) -1, 0);
#else
		PREFIX(_interrupt)((unsigned int) -1);
#endif
}
...
static void PREFIX86(_iret)(void)    /* Opcode 0xcf */
{
	...
	if (I.IF && I.irq_state)
#ifdef V20
		PREFIX(_interrupt)((unsigned) -1, 0);
#else
...
static void PREFIX86(_inax)(void)    /* Opcode 0xe5 */
{
	...
	I.regs.b[AL] = read_port(port); //read_port(port);
    ...
    case 0x38:  /* IDIV AL, Ew */
		ICOUNT -= (ModRM >= 0xc0) ? cycles.idiv_r8 : cycles.idiv_m8;
		{
			...
			if (tmp)
			{
				...
				{
					I.regs.b[AL] = (unsigned char) result;	// MPO : added cast to get rid of compiler warning
					I.regs.b[AH] = tmp2;
				}
			}
			else
