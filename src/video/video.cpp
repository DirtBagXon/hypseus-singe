...
void take_screenshot(SDL_Overlay *yuvimage)
{
    ...
    SDL_Surface *rgbimage = SDL_CreateRGBSurface(SDL_SWSURFACE, yuvimage->w, yuvimage->h,
    ...
    if (rgbimage) {
        ...
        if (yuvimage->format == SDL_YV12_OVERLAY) {
            ...
            for (int row = 0; row < yuvimage->h; row += 2) {
                ...
                for (int col = 0; col < yuvimage->w; col += 2) {
                    // get a 2x2 block of pixels and store them due to YUV
                    // structure
                    // upper left
                    yuv2rgb(&cur_color, *Y, *U, *V);
                    cur_pixel[0] = SDL_MapRGB(rgbimage->format, cur_color.r,
                                              cur_color.g, cur_color.b);

                    // upper right
                    yuv2rgb(&cur_color, *(Y + 1), *U, *V);
                    cur_pixel[1] = SDL_MapRGB(rgbimage->format, cur_color.r,
                                              cur_color.g, cur_color.b);

                    // lower left
                    yuv2rgb(&cur_color, *(Y + yuvimage->pitches[0]), *U, *V);
                    cur_pixel[0 + rgbimage->w] =
                        SDL_MapRGB(rgbimage->format, cur_color.r, cur_color.g, cur_color.b);

                    // lower right
                    yuv2rgb(&cur_color, *(Y + yuvimage->pitches[0] + 1), *U, *V);
                    cur_pixel[1 + rgbimage->w] =
                        SDL_MapRGB(rgbimage->format, cur_color.r, cur_color.g, cur_color.b);
                ...
                V += v_extra;
                U += u_extra;
                cur_pixel += rgbimage->w; // move down one row (FIXME: I think
                                          // this should use pitch)
            ...
            for (int row = 0; row < yuvimage->h; row++) {
                ...
                for (int col = 0; col < yuvimage->w; col += 2) {
                    ...
                    // right
                    yuv2rgb(&cur_color, *(pixel_ptr + 2), *(pixel_ptr + 1),
                            *(pixel_ptr + 3));
                    cur_pixel[1] =
                        SDL_MapRGB(rgbimage->format, (cur_color.r & 0xFE),
                                   (cur_color.g & 0xFE), (cur_color.b & 0xFE));

                    cur_pixel += 2; // move 2 pixels over
                    pixel_ptr += 4; // move 4 bytes over
                }
                line_ptr += yuvimage->pitches[0];
                cur_pixel += (rgbimage->pitch - (rgbimage->w * 4)) /
                             4; // this look complicated, but the pitch should
                                // be the width * 4 so it's just a precaution
                // NOTE : if the pitch is not a multiple of 4, then we've got
                // problems..
            }
        ...
        SDL_FreeSurface(rgbimage); // de-allocate surface, as we no longer need
                                   // it
    ...
    if (bSaveYUV && (yuvimage->format == SDL_YV12_OVERLAY)) {
        ...
        // Most people won't need or want these files, but I have use of them
        // for testing
        Uint8 *Y = yuvimage->pixels[0];
        Uint8 *V = yuvimage->pixels[1];
        Uint8 *U = yuvimage->pixels[2];
...
{
    SDL_Surface *rgbimage = SDL_CreateRGBSurface(SDL_SWSURFACE, g_vid_width, g_vid_height,
                                                 32, 0xFF, 0xFF00, 0xFF0000, 0);
    ...
    if (rgbimage) {
        ...
        if (bufTmp) {
            ...
            // OpenGL goes bottom to top, and the SDL Surface goes top to
            // bottom,
            ...
            printline("ERROR: take_screenshot_GL memory allocation failed");
