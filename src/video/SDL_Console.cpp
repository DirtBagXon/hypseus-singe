...
#ifdef _MSC_VER
#pragma warning(disable : 4244) // disable the warning about possible loss of
                                // data
#endif
...
static int ConsoleScrollBack = 0; /* How much the users scrolled back in the
                                     console */
...
static SDL_Surface *Border = NULL;  /* Sides, bottom and corners of the console
                                       border */
static SDL_Surface *BackgroundImage = NULL; /* Background image for the console
                                               */
static SDL_Surface *InputBackground; /* Dirty rectangle to draw over behind the
                                        users background */
...
void ConsoleEvents(SDL_Event *event)
{
    ...
    static int StringLocation = 0; /* Current character location in the current
                                      string */
    static int CommandScrollBack = 0; /* How much the users scrolled back in the
                                         command lines */
    ...
    case SDL_KEYDOWN:
        ...
        case SDLK_UP:
            if (CommandScrollBack < TotalCommands) {
                /* move back a line in the command strings and copy the command
                 * to the current input string */
                CommandScrollBack++;
        ...
        case SDLK_DOWN:
            if (CommandScrollBack > 0) {
                /* move forward a line in the command strings and copy the
                 * command to the current input string*/
                CommandScrollBack--;
                memset(ConsoleLines[0], 0, CHARS_PER_LINE);
                strcpy(ConsoleLines[0], CommandLines[CommandScrollBack]);
        ...
        case SDLK_BACKSPACE:
            if (StringLocation > 0) {
                ...
                SDLDrawText(ConsoleLines[0], ConsoleSurface, FontNumber, 4,
...
void UpdateConsole()
{
    ...
    for (loop = 0; loop < Screenline; loop++)
        SDLDrawText(ConsoleLines[Screenline - loop + ConsoleScrollBack],
                    ConsoleSurface, FontNumber, 4, loop * FontHeight(FontNumber));

    if (ConsoleScrollBack > 0)
        for (loop = 4; loop < OutputScreen->w - FontWidth(FontNumber) * 2;
             loop += FontWidth(FontNumber) * 2)
            SDLDrawText("^ ", ConsoleSurface, FontNumber, loop,
                        ConsoleSurface->h - FontHeight(FontNumber));
    else
        SDLDrawText(ConsoleLines[0], ConsoleSurface, FontNumber, 4,
                    ConsoleSurface->h - FontHeight(FontNumber));
...
int ConsoleInit(const char *FontName, SDL_Surface *DisplayScreen, int lines)
{
    ...
    Temp = SDL_AllocSurface(SDL_SWSURFACE, OutputScreen->w, OutputScreen->h / 2,
    ...
    Temp = SDL_AllocSurface(SDL_SWSURFACE, OutputScreen->w, FontHeight(FontNumber),
                            OutputScreen->format->BitsPerPixel, 0, 0, 0, 0);
    if (Temp == NULL) {
        printf("Error Console.c:ConsoleInit()\n\tCouldn't create the input "
               "background\n");
        return 1;
    }
...
/* Outputs text to the console (in game and stdout), up to 256 chars can be
 * entered
...
int SetConsoleBorder(const char *border)
{
    ...
    if (Border != NULL) SDL_FreeSurface(Border);
...
int ConsoleBackground(const char *image, int x, int y)
{
    ...
    if (BackgroundImage != NULL) SDL_FreeSurface(BackgroundImage);
