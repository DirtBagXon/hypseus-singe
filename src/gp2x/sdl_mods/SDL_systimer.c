 ...
 * Original: ./src/timer/linux/SDL_systimer.c
 */
/*
...
#ifdef SAVE_RCSID
static char rcsid =
 "@(#) $Id: SDL_systimer.c,v 1.10 2004/01/04 16:49:19 slouken Exp $";
#endif
...
// MPO : give SDL_GetTicks access to GP2X registers
extern unsigned short volatile *g_p16GP2XRegs;
...
Uint32 SDL_GetTicks (void)
{
	...
	// start MPO
#ifndef ENABLE_GP2XVIDEO
	struct timeval now;
	gettimeofday(&now, NULL);
	ticks=(now.tv_sec-start.tv_sec)*1000+(now.tv_usec-start.tv_usec)/1000;
#else
// UPDATE : we can't use statics because this function must be THREAD SAFE!
/*
	// GP2X SECTION : fixing dog-slow SDL_GetTicks on GP2X
	static Uint32 uLastTicks = 0;
	static Uint32 uExtraTicks = 0;
	*/

	// C000:0A00 = 32-bit timer which 'ticks' at 0.27uSecs (1/7.3728MHz)
	// Floating point math is slow, so I round 7372.8 to 7373; slightly inaccurate, but hopefully won't matter.
	// The additional speed benefits are _well_ worth it, IMO.
	// This will wraparound every 9.7 minutes. :(
	ticks = ((unsigned int *) g_p16GP2XRegs)[0x0A00>>2] / 7373;

	/*
	// if we have wrapped around since last time, then add to our extra ticks to maintain accuracy
	if (ticks < uLastTicks)
	{
#ifdef GP2X_DEBUG
		printf("SDL_GetTicks() : timer wraparound detected! (this should happen every 9.7 minutes)\n");
#endif // GP2X_DEBUG
		// since we've wrapped around, we know this many milliseconds needs to be added to the front
		uExtraTicks += 582527;	// 582527 = (2^32 / 7373) (rounded up to make sure we don't go "backward" in time)
	}

	ticks += uExtraTicks;	// compensate for wraparound

	uLastTicks = ticks;
	*/
#endif // ENABLE_GP2XVIDEO
	// end MPO, SDL_GetTicks optimization
