...
bool g_char_buf_filled      = false; // whether we have any character waiting to be
                                     // read in our buffer
#endif
...
unsigned char serial_rx()
{
    ...
    if (g_char_buf_filled) {
        ...
        printline("SERIAL ERROR : serial port had no character waiting to be "
                  "read, did you check??");
    }
                ...
                while (fWaitingOnRead) {
                    ...
                    case WAIT_OBJECT_0:
                        ...
                        if (!GetOverlappedResult(h, &osReader, &dwRead, FALSE)) {
                            ...
                            // MATT : I changed this from (ch != 0) to try to
                            // fix a problem
                            // with a byte of '00' not being able to be read
...
// You should check to see whether a character is available first before calling
// this
unsigned char serial_rx()
{
    unsigned char result = 0;
    ...
    if (serial_rx_char_waiting()) {
        if (read(fd, &result, 1) != 1) {
            printline("SERIAL ERROR : Tried to read a byte but no byte was "
                      "available.  Did your code check first??");
            result = 0;
        }
    } else {
        printline("SERIAL ERROR : Tried to read a byte and no byte was "
                  "available, grr!  Check before reading!");
    }
...
// returns a 1 if successful, 0 if error
{
    ...
    if (tcsetattr(fd, TCSANOW, &oldterm) == -1) {
        printline(
            "Error restoring settings to their originals in serial_close()");
    }
    close(fd);
}
