...
local int unzlocal_getShort(mpo_io *fin, uLong *pX) // MPO
{
    uLong x;
    int i = 0;
    int err;
...
local int unzlocal_getLong(mpo_io *fin, uLong *pX) // MPO
{
    uLong x;
    int i = 0;
    int err;
    ...
    mpo_io *fin) // MPO
{
    ...
    uSizeFile = (uLong)fin->size;   // WARNING : we assume that we won't handle
                                    // any huge .zip files
    // stop MPO
...
extern unzFile ZEXPORT unzOpen(const char *path)
{
    ...
    //    fin=fopen(path,"rb");
    //	if (fin==NULL)
    fin = mpo_open(path, MPO_OPEN_READONLY);
    if (!fin)
        // stop MPO
        return NULL;
    ...
    if (!mpo_seek(central_pos, MPO_SEEK_SET, fin)) // MPO
        err = UNZ_ERRNO;
...
OF((unzFile file, unz_file_info *pfile_info, unz_file_info_internal *pfile_info_internal,
...
local int
unzlocal_GetCurrentFileInfoInternal(unzFile file, unz_file_info *pfile_info,
                                    ...
                                    char *szComment, uLong commentBufferSize)
{
    ...
    if (!mpo_seek(s->pos_in_central_dir + s->byte_before_the_zipfile, MPO_SEEK_SET, s->file)) // MPO
        err = UNZ_ERRNO;
    ...
    if (unzlocal_getShort(s->file, &file_info.version_needed) != UNZ_OK) // MPO
        err = UNZ_ERRNO;

    if (unzlocal_getShort(s->file, &file_info.flag) != UNZ_OK) // MPO
        err = UNZ_ERRNO;

    if (unzlocal_getShort(s->file, &file_info.compression_method) != UNZ_OK) // MPO
        err = UNZ_ERRNO;

    if (unzlocal_getLong(s->file, &file_info.dosDate) != UNZ_OK) // MPO
        err = UNZ_ERRNO;

    unzlocal_DosDateToTmuDate(file_info.dosDate, &file_info.tmu_date);

    if (unzlocal_getLong(s->file, &file_info.crc) != UNZ_OK) // MPO
        err = UNZ_ERRNO;

    if (unzlocal_getLong(s->file, &file_info.compressed_size) != UNZ_OK) // MPO
        err = UNZ_ERRNO;

    if (unzlocal_getLong(s->file, &file_info.uncompressed_size) != UNZ_OK) // MPO
        err = UNZ_ERRNO;

    if (unzlocal_getShort(s->file, &file_info.size_filename) != UNZ_OK) // MPO
        err = UNZ_ERRNO;

    if (unzlocal_getShort(s->file, &file_info.size_file_extra) != UNZ_OK) // MPO
        err = UNZ_ERRNO;

    if (unzlocal_getShort(s->file, &file_info.size_file_comment) != UNZ_OK) // MPO
        err = UNZ_ERRNO;

    if (unzlocal_getShort(s->file, &file_info.disk_num_start) != UNZ_OK) // MPO
        err = UNZ_ERRNO;

    if (unzlocal_getShort(s->file, &file_info.internal_fa) != UNZ_OK) // MPO
        err = UNZ_ERRNO;

    if (unzlocal_getLong(s->file, &file_info.external_fa) != UNZ_OK) // MPO
        err = UNZ_ERRNO;

    if (unzlocal_getLong(s->file, &file_info_internal.offset_curfile) != UNZ_OK) // MPO
        err = UNZ_ERRNO;
    ...
    if ((err == UNZ_OK) && (szFileName != NULL)) {
        ...
        if ((file_info.size_filename > 0) && (fileNameBufferSize > 0))
            ...
            if (!mpo_read(szFileName, uSizeRead, NULL, s->file) || s->file->eof) // MPO
                err = UNZ_ERRNO;
        lSeek -= uSizeRead;
    }
...
/*
  ...
  return UNZ_OK if there is no problem.
*/
extern int ZEXPORT unzGetCurrentFileInfo(unzFile file, unz_file_info *pfile_info,
                                         char *szFileName, uLong fileNameBufferSize,
                                         void *extraField, uLong extraFieldBufferSize,
                                         char *szComment, uLong commentBufferSize)
{
                                                   ...
                                                   uInt *psize_local_extrafield)
{
    ...
    *psize_local_extrafield   = 0;
    ...
    if (!mpo_seek(s->cur_file_info_internal.offset_curfile + s->byte_before_the_zipfile,
...
extern int ZEXPORT unzOpenCurrentFile(unzFile file)
{
    ...
    if (unzlocal_CheckCurrentFileCoherencyHeader(s, &iSizeVar, &offset_local_extrafield,
...
extern int ZEXPORT unzReadCurrentFile(unzFile file, voidp buf, unsigned len)
{
    ...
    while (pfile_in_zip_read_info->stream.avail_out > 0) {
        if ((pfile_in_zip_read_info->stream.avail_in == 0) &&
            ...
            if (!mpo_seek(pfile_in_zip_read_info->pos_in_zipfile +
                              pfile_in_zip_read_info->byte_before_the_zipfile,
                          MPO_SEEK_SET,
                          pfile_in_zip_read_info->file)) // MPO
                return UNZ_ERRNO;
            //			if
            //(fread(pfile_in_zip_read_info->read_buffer,uReadThis,1,
            //                         pfile_in_zip_read_info->file)!=1)
            if (!mpo_read(pfile_in_zip_read_info->read_buffer, uReadThis, NULL,
                          pfile_in_zip_read_info->file) ||
                pfile_in_zip_read_info->file->eof) // MPO
                return UNZ_ERRNO;
...
extern int ZEXPORT unzGetLocalExtrafield(unzFile file, voidp buf, unsigned len)
{
    ...
    if (!mpo_seek(pfile_in_zip_read_info->offset_local_extrafield + pfile_in_zip_read_info->pos_local_extrafield,
                  MPO_SEEK_SET,
                  pfile_in_zip_read_info->file)) // MPO
        return UNZ_ERRNO;

    //	if (fread(buf,(uInt)size_to_read,1,pfile_in_zip_read_info->file)!=1)
    if (!mpo_read(buf, size_to_read, NULL, pfile_in_zip_read_info->file) ||
        pfile_in_zip_read_info->file->eof) // MPO
        return UNZ_ERRNO;
...
extern int ZEXPORT unzGetGlobalComment(unzFile file, char *szComment, uLong uSizeBuf)
{
    ...
    if (!mpo_seek(s->central_pos + 22, MPO_SEEK_SET, s->file)) // MPO
        return UNZ_ERRNO;

    if (uReadThis > 0) {
        *szComment = '\0';
        //	  if (fread(szComment,(uInt)uReadThis,1,s->file)!=1)
        if (!mpo_read(szComment, uReadThis, NULL, s->file) || s->file->eof) // MPO
            return UNZ_ERRNO;
    }
