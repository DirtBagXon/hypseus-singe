...
int idle_handler(void *surface)
{
	...
	for (i = 0; drivers[i].name != NULL; i++)
		if (strcmp (drivers[i].name, "null") == 0)
			s_video_output = drivers[i].open();	// open 'null' driver (we just pass decoded frames to parent thread)
    ...
    while(1)
    {
		...
		case STATE_PICTURE:
			...
			if (s_video_output->start_fbuf)
				s_video_output->start_fbuf(s_video_output, info->current_fbuf->buf, info->current_fbuf->id);
...
// and that mpeg2_reset() has been called
