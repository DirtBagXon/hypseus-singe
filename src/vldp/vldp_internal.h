...
extern int s_frames_to_skip; // how many frames to skip before rendering the
                             // next frame (used for P and B frames seeking)
extern int s_frames_to_skip_with_inc; // how many frames to skip while
...
extern unsigned int s_uSkipAllCount; // how many frames we've skipped when
...
extern Uint32 s_uFramesShownSinceTimer; // how many frames should've been
                                        // rendered (relative to s_timer) before
                                        // we advance
...
extern SDL_Overlay *s_hw_overlay; // if the game uses video overlay, we can't
                                  // modify our buffers, so we have to
...
extern unsigned int s_stall_per_frame; // how many frames to stall per frame
