...
#else
// the bitrate is so low on the gp2x files that having a huge buffer really hurts performance
// TODO : automatically calculate ideal bitrate based on .dat file
...
extern void YV12Callback(const struct yv12_s *cpYV12);
extern void YUY2Callback(const struct yuy2_s *cpYUY2);
extern void ResolutionCallback(unsigned int X, unsigned int Y);
...
int idle_handler(void *surface)
{
	...
	mp2_920_init(&callbacks, VLDP_TRUE);

	// ok, parent thread can start sending commands now that mp2_920_init is done
	// (the main issue here is the get ticks function not being ready until mp2_920_init returns)
	g_ack_init_done = VLDP_TRUE;
...
{
	mp2_920_decode_mpeg(g_header_buf, g_header_buf + g_header_buf_size);
...
//		end = g_buffer + fread (g_buffer, 1, BUFFER_SIZE, g_mpeg_handle);
		...
		if (g_buffer != end)
		{
			...
			mp2_920_decode_mpeg(g_buffer, end);
		}
