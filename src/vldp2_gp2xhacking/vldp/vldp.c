...
unsigned int g_ack_init_done = VLDP_FALSE;	// the internal thread sets this when it's safe for vldp_init to return
...
const struct vldp_out_info *vldp_init(const struct vldp_in_info *in_info)
{
	...
	if (private_thread)
	{
		// wait for child thread to finish starting up
		while (g_ack_init_done != VLDP_TRUE)
		{
			usleep(0);
		}
		p_initialized = 1;
