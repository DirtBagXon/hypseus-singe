...
#include <inttypes.h>
#include <mpo_lib.h>	// replace memcmp
...
const uint8_t default_intra_quantizer_matrix[64] ATTR_ALIGN(16) = {
    8,
    16, 16,
    19, 16, 19,
    22, 22, 22, 22,
...
static int sequence_ext (mpeg2dec_t * mpeg2dec)
{
	...
	*/
	...
	sequence->frame_period =
		dzzIntDivide(sequence->frame_period * ((buffer[5]&31)+1), (((buffer[5]>>2)&3)+1));
...
static inline void finalize_sequence (sequence_t * sequence)
{
	...
	int tmp = width;
	//width = height % tmp;
	width = dzzIntMod(height, tmp);
	height = tmp;
    }
//    sequence->pixel_width /= height;
//    sequence->pixel_height /= height;
	sequence->pixel_width = dzzIntDivide(sequence->pixel_width, height);
	sequence->pixel_height = dzzIntDivide(sequence->pixel_height, height);
}
...
void mpeg2_header_sequence_finalize (mpeg2dec_t * mpeg2dec)
{
    ...
    if (!MPO_MEMCMP (&(mpeg2dec->sequence), sequence, sizeof (sequence_t)))
